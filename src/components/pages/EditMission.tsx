...
const EditMission: React.FC = () => {
    ...
    const fetchMission = async () => {
      try {
        ...
        setFormData({
          ...
          landingDate: new Date(data.landingDate).toISOString().split('T')[0],
          launchLatitude: data.launchLatitude.toString(),
          launchLongitude: data.launchLongitude.toString(),
          landingLatitude: data.landingLatitude.toString(),
          landingLongitude: data.landingLongitude.toString(),
        });
      } catch {
        setErrors({ submit: 'Ошибка при загрузке миссии' });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const validateForm = () => {
    ...
    if (!formData.landingDate) newErrors.landingDate = 'Дата посадки обязательна';
    if (!formData.launchSite) newErrors.launchSite = 'Место запуска обязательно';
    if (!formData.landingSite) newErrors.landingSite = 'Место посадки обязательно';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ submit: 'Ошибка при обновлении миссии' });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto mt-8">
      <div className="bg-white shadow-lg rounded-lg overflow-hidden">
        <div className="p-6">
          <h1 className="text-3xl font-bold mb-6 text-gray-800">Редактирование миссии</h1>

          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-700">Название миссии</label>
                <input
                  type="text"
                  name="name"
                  value={formData.name}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">Дата запуска</label>
                <input
                  type="date"
                  name="launchDate"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">Дата посадки</label>
                <input
                  type="date"
                  name="landingDate"
                ...
                />
                {errors.landingDate && <p className="mt-1 text-sm text-red-500">{errors.landingDate}</p>}
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700">Место запуска</label>
                <input
                  type="text"
                  name="launchSite"
                ...
                />
                {errors.launchSite && <p className="mt-1 text-sm text-red-500">{errors.launchSite}</p>}
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700">Широта запуска</label>
                <input
                  type="number"
                  step="0.000001"
                  name="launchLatitude"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">Долгота запуска</label>
                <input
                  type="number"
                  step="0.000001"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">Место посадки</label>
                <input
                  type="text"
                  name="landingSite"
                ...
                />
                {errors.landingSite && <p className="mt-1 text-sm text-red-500">{errors.landingSite}</p>}
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700">Широта посадки</label>
                <input
                  type="number"
                  step="0.000001"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">Долгота посадки</label>
                <input
                  type="number"
                  step="0.000001"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">Лунный модуль</label>
                <input
                  type="text"
                  name="lunarModule"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">Управляющий модуль</label>
                <input
                  type="text"
                  name="commandModule"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Участники миссии</label>
              {formData.crewMembers.map((member, index) => (
                <div key={index} className="flex gap-2 mb-2">
                  <input
                    type="text"
                    value={member}
                    onChange={(e) => handleCrewMemberChange(index, e.target.value)}
            ...
            {errors.submit && (
              <div className="p-3 bg-red-100 text-red-700 rounded-md">{errors.submit}</div>
            )}
